...
const loadInitialData = async (query, variables = {}) =>{
	...
	console.log("111AAAA: " + JSON.stringify({ query, variables }));
...
const deleteBook = async (books, id)=>{
	const mutation = `{ deleteBook(id: ${id}){successful}} `;
	const variables = {};
	console.log("q " + mutation);
	console.log("q " + JSON.stringify(mutation));
	console.log("v " + variables);
	console.log("v " + JSON.stringify(variables));
	console.log("AAAA: " + JSON.stringify({ mutation, variables }));
	const response = await fetch('/api/', {
    	method: 'POST',
     	headers: { 'Content-Type': 'application/json' },
